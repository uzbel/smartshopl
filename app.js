...
function navigate(page) {
  ...
  const renders = {
    ...
    sales: renderSalesForm, today: renderToday,
    expenses: renderExpenses, owner: renderOwnerExp,
    report: renderReport, history: renderHistory,
    stock: renderStock, settings: renderSettings
  };
...
function renderDashboard() {
  ...
  const cogs = sales.filter(s=>s.date.startsWith(ym)).reduce((a,x)=>a+(x.costTotal||0),0);
  ...
  document.getElementById('db-recent').innerHTML = sales.slice(-6).reverse().map(s=>`
    <tr>
    ...
    </tr>`).join('') || '<tr><td colspan="4" style="text-align:center;color:var(--text3);padding:24px">Sotuv yo\'q</td></tr>';
...
function renderProducts(search='') {
  ...
  document.getElementById('prod-tbody').innerHTML = prods.map(p => {
    ...
    return `<tr>
      ...
      <td><span class="${sc} badge">${fmtN(p.stock)} ${p.unit}</span></td>
...
function renderSalesForm() {
  ...
  sel.innerHTML = '<option value="">Mahsulot tanlang</option>' +
    prods.map(p=>`<option value="${p.id}" data-price="${p.sellPrice}" data-cost="${p.buyPrice}" data-stock="${p.stock}">${p.name} — ${fmt(p.sellPrice)} (${fmtN(p.stock)} ${p.unit})</option>`).join('');
}
...
function calcSaleTotal() {
  ...
  if (opt && opt.dataset.price)
    document.getElementById('sale-total-preview').textContent = fmt(opt.dataset.price * qty);
}
...
function addSale() {
  ...
  if (qty > parseFloat(opt.dataset.stock)) { toast('Omborda yetarli mahsulot yo\'q!','var(--danger)'); return; }
  ...
  sales.push({
    id:newId, date:DB.today(),
    time:`${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`,
    productId:parseInt(opt.value), productName:opt.text.split(' — ')[0],
...
function renderToday() {
  ...
  const cogs  = sales.reduce((a,x)=>a+(x.costTotal||0),0);
  ...
  document.getElementById('today-tbody').innerHTML = sales.slice().reverse().map(s=>`
    <tr>
    ...
    </tr>`).join('') || '<tr><td colspan="7"><div class="empty-state"><div class="empty-icon">&#128722;</div><div class="empty-text">Bugun sotuv yo\'q</div></div></td></tr>';
}
...
function renderExpenses() {
  ...
  const mexp = exps.filter(e => e.date.startsWith(ym));
  document.getElementById('exp-total').textContent = fmt(mexp.reduce((a,x)=>a+x.amount,0));
  document.getElementById('exp-count').textContent = mexp.length;
  document.getElementById('exp-tbody').innerHTML = exps.slice().reverse().map(e=>`
    <tr>
    ...
    </tr>`).join('') || '<tr><td colspan="5"><div class="empty-state"><div class="empty-icon">&#128178;</div><div class="empty-text">Xarajat yo\'q</div></div></td></tr>';
}
...
function renderOwnerExp() {
  ...
  const mexp = exps.filter(e => e.date.startsWith(ym));
  document.getElementById('oe-total').textContent = fmt(mexp.reduce((a,x)=>a+x.amount,0));
  document.getElementById('oe-tbody').innerHTML = exps.slice().reverse().map(e=>`
    <tr>
      <td style="font-family:'JetBrains Mono',monospace;color:var(--text3)">${e.date}</td>
      <td><b>${e.desc}</b></td>
      <td>${e.note||'—'}</td>
...
function renderReport() {
  ...
  document.getElementById('rp-gross').textContent    = fmt(gross);
  ...
  document.getElementById('rp-net').className        = 'stat-value ' + (net>=0?'green':'red');

  const topMap = {};
  sales.filter(s=>s.date.startsWith(ym)).forEach(s=>{ topMap[s.productName]=(topMap[s.productName]||0)+s.total; });
...
function renderHistory(search='') {
  ...
  if (search) sales = sales.filter(s =>
    s.productName.toLowerCase().includes(search.toLowerCase()) || s.date.includes(search)
  );
  document.getElementById('hist-tbody').innerHTML = sales.map(s=>`
    <tr>
      ...
      <td style="font-family:'JetBrains Mono',monospace;color:var(--accent3)">${fmt(s.total)}</td>
      <td>${s.note||'—'}</td>
    </tr>`).join('') || '<tr><td colspan="6"><div class="empty-state"><div class="empty-icon">&#128203;</div><div class="empty-text">Sotuv tarixi yo\'q</div></div></td></tr>';
}
...
function renderStock() {
  ...
  const val   = prods.reduce((a,p) => a + p.stock*p.buyPrice, 0);
  document.getElementById('stk-total').textContent = prods.length;
  document.getElementById('stk-low').textContent   = low.length;
  document.getElementById('stk-value').textContent = fmt(val);
  document.getElementById('stk-tbody').innerHTML = prods.map(p=>{
    const sc  = p.stock<=3?'badge-red':p.stock<=10?'badge-yellow':'badge-green';
    const pct = Math.min(100, p.stock/50*100);
    const clr = p.stock<=3?'var(--danger)':p.stock<=10?'var(--warning)':'var(--accent3)';
    return `<tr>
      <td><b>${p.name}</b></td>
      <td><span class="${sc} badge">${fmtN(p.stock)} ${p.unit}</span>
        <div class="profit-bar" style="max-width:120px"><div class="profit-fill" style="width:${pct}%;background:${clr}"></div></div>
      </td>
      <td style="font-family:'JetBrains Mono',monospace">${fmt(p.buyPrice)}</td>
      <td style="font-family:'JetBrains Mono',monospace;color:var(--accent2)">${fmt(p.stock*p.buyPrice)}</td>
      <td><button class="btn btn-outline btn-sm" onclick="restockProduct(${p.id})">+ Zaxira qo'sh</button></td>
    </tr>`;
  }).join('');
}
...
function clearAllData() {
  ...
  ['products','sales','owner_exp','expenses','shop_name','owner_name','shop_phone','seeded'].forEach(k=>localStorage.removeItem('ss_'+k));
  location.reload();
}
...
function updateClock() {
  ...
  if (el) el.textContent = `${days[d.getDay()]}, ${d.getDate()} ${months[d.getMonth()]} ${d.getFullYear()} | ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;
}
...
document.addEventListener('DOMContentLoaded', () => {
  const ym = document.getElementById('report-ym');
  if (ym) ym.value = DB.currentYM();
  navigate('dashboard');
});
